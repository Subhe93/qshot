...
function drawCroppedImage(canvas: HTMLCanvasElement, item: any, index: number) {
  ...
  img.onload = () => {
    ...
    const canvasHeight = containerRect.height || 225; // 16:9 ratio

    // Set canvas size to match container (16:9 aspect ratio for swiper)
  ...
  img.onerror = (error) => {
    ...
    const containerRect = canvas.getBoundingClientRect();
    const canvasWidth = containerRect.width || 400;
    const canvasHeight = containerRect.height || 225;
    ...
    ctx.font = `${Math.max(14, canvasWidth / 25)}px Arial`;
      ...
      >
        <NuxtLink target="_blank" class="flex items-center justify-left">
          <div class="flex w-full rounded-l-2xl rounded-r-2xl overflow-hidden">
            <div
              class="w-full aspect-[16/9] rounded-md overflow-hidden relative border border-gray-800/30"
            >
              <canvas
                v-if="
                  item.rect &&
